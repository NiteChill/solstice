...
public class AuthController {
  ...
  private ResponseCookie generateCookie(String refreshToken) {
    ...
    return ResponseCookie.from("solstice_rt", refreshToken)
      .httpOnly(true)
      .secure(isProd)
  ...
  private ResponseCookie generateSessionCookie(long maxAge) {
    boolean isProd = "prod".equalsIgnoreCase(env);
    return ResponseCookie.from("solstice_session", "true")
      .httpOnly(false)
      .secure(isProd)
      .sameSite(isProd ? "Strict" : "Lax")
      .path("/")
  ...
  public ResponseEntity<AuthenticationResponse> register(
    ...
    return ResponseEntity.status(HttpStatus.CREATED)
      .header(
        ...
        generateCookie(result.refreshToken()).toString()
      )
      .header(
        HttpHeaders.SET_COOKIE,
        generateSessionCookie(refreshExpiration / 1000).toString()
      )
  ...
  public ResponseEntity<?> logout(
    ...
    ResponseCookie deleteCookie = ResponseCookie.from("solstice_rt", "")
      .httpOnly(true)
      .secure("prod".equalsIgnoreCase(env))
      .sameSite("prod".equalsIgnoreCase(env) ? "Strict" : "Lax")
      .path("/api/v1/auth")
      .maxAge(0)
      .build();
    ...
    return ResponseEntity.ok()
      ...
      .header(HttpHeaders.SET_COOKIE, generateSessionCookie(0).toString())
      .build();
  }
  ...
  public ResponseEntity<AuthenticationResponse> refresh(
    ...
    String userAgent = servletRequest.getHeader("User-Agent");
    String ipAddress = servletRequest.getRemoteAddr();
    AuthResult result = refreshTokenService.rotateToken(
      refreshToken,
      userAgent,
      ipAddress
    );
  ...
  public List<SessionResponse> getSessions(
    ...
    Long currentSessionId = jwtService.extractClaim(token, claims ->
